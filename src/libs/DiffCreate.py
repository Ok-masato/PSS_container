...
class DiffCreate:
    ...
    def trace_save(self, name, pos):
        ...
        wb = openpyxl.load_workbook('train_data/sample.xlsx')
        sheet = wb['sheet1']
        maxgyou=sheet.max_row + 1
        sheet.cell(row = maxgyou, column = 1).value = dt_now
        sheet.cell(row = maxgyou, column = 2).value = self.input_dir + str(self.num)  + "-" + str(self.camera) + ".jpg"
        sheet.cell(row = maxgyou, column = 3).value = self.obj_db + name
        sheet.cell(row = maxgyou, column = 4).value = str(pos[0])
        sheet.cell(row = maxgyou, column = 5).value = str(pos[1])
        sheet.cell(row = maxgyou, column = 6).value = str(pos[2])
        sheet.cell(row = maxgyou, column = 7).value = str(pos[3])
        wb.save('train_data/sample.xlsx')
    ...
    def detect_contour(self):
        ...
        # ぼかし
        diff_img = cv2.cvtColor(cv2.GaussianBlur(diff_img, (5, 5), 2), cv2.COLOR_BGR2GRAY)
